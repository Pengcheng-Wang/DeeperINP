...
function CIUserScoreSoftPredictor:testScorePredOnTestDetOneEpoch(_evalStat)
    -- just in case:
    collectgarbage()
    ...
    if string.sub(self.opt.uppModel, 1, 4) == 'rnn_' then
        ...
        for evl_ite = 1, evlCnt do
            ...
            for i=1, #self.rnnRealUserDataEndsTest do
                ...
                if _evalStat then
                    local lp, rin = torch.max(nll_rewards[self.opt.lstmHist][1][i]:squeeze(), 1)
    ...
    elseif string.sub(self.opt.uppModel, 1, 4) == 'cnn_' then
        ...
        for evl_ite = 1, evlCnt do
            ...
            for i=1, #self.cnnRealUserDataEndsTest do
                ...
                if _evalStat then
                    local lp, rin = torch.max(nll_rewards[1][i]:squeeze(), 1)
