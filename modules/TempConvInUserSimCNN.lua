...
--- v5, v6, v7, v8 corresponds to v1, v2, v3, v4 respectively, also make the difference that
...
function TempConvUserSimCNN:CreateCNNModule(inputSize, outputSize, cnn_layers, kernel_width, dropout_rate, frame_cnt, version)
    assert(version == 'v1' or version == 'v2' or version == 'v3' or version == 'v4' or version == 'v5' or version == 'v6' or version == 'v7' or version == 'v8',
            'Convolution module in player simulation modeling can only be v1, v2 v3 or v4')
    ...
    local _useBatchNorm = true
    if version == 'v5' or version == 'v6' or version == 'v7' or version == 'v8' then _useBatchNorm = false end

    local input = nn.Identity()()
    local outputs = {}
    local _dropped_in = input
    if (version == 'v4' or version == 'v8') and dropout_rate > 0 then _dropped_in = nn.Dropout(dropout_rate)(input) end
    ...
    for layer_idx=1, cnn_layers do
        ...
        local _batchNorm = _useBatchNorm and nn.TemporalBatchNormalizationW(frame_cnt)(_conv) or _conv
        local _outConv = _batchNorm     -- v1
        if version == 'v2' or version == 'v6' then
            _outConv = nn.CAddTable()({_batchNorm, _input}) -- residual connection added for each convolution layer for version v2, v6: added with value before convolution
        elseif version == 'v3' or version == 'v7' then
            ...
            local _batchNorm_2 = _useBatchNorm and nn.TemporalBatchNormalizationW(frame_cnt)(_conv_2) or _conv_2
            _outConv = nn.CAddTable()({_batchNorm_2, _input}) -- residual connection added for each convolution layer for version v2, v6
        elseif version == 'v4' or version == 'v8' then
            _outConv = nn.CAddTable()({_batchNorm, _dropped_in}) -- residual connection added for each convolution layer for version v4, v8: direct adding with original input
        end
